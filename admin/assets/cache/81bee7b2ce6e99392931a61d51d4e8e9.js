...
var itemUrl = {
    stock_url:config.base_url+"opd/api/opd_api/getStock",
    modeldrug_url:config.base_url+"opd/api/opd_modal/model_list_drug",
    modelother_url:config.base_url+"opd/api/opd_modal/model_list_other",
    editmodeldrug_url:config.base_url+"opd/api/opd_modal/edit_model_list_drug",
    editmodelother_url:config.base_url+"opd/api/opd_modal/edit_model_list_other",
    editmodelRelateddrug_url:config.base_url+"opd/api/opd_modal/edit_model_related_list_drug",
    saleitem_url:config.base_url+'opd/api/opd_api/saleitem',
    addpricing_url:config.base_url+'opd/api/opd_api/',
}
            ...
            if(data.length > 0){
                for (var i = 0; i < data.length; i++) {
                    ...
                    if(data[i].type == 'main'){
                        ...
                        '</div>';
                        btnDelete = '<button onclick="deleteItem(\''+data[i].type+'\','+data[i].main_id+');"  type="button" class="btn btn-danger btn-block mdi mdi-pencil text-light"> ลบ </button>';
                    }else{                                          
                        selectbtn = '<div class="box-label">'+
...
function deleteListStock(id){   
   swal({   
        ...
        if (isConfirm) {     
            ...
            $.ajax({
                type: 'POST',
                url: Config.base_url+'opd/api/opd_api/deletemain',
                data: {
                    'id': id
                },
                success: function (data) {
...
function deleteListStockRelated(id){
    swal({   
        ...
        if (isConfirm) {     
            ...
            $.ajax({
                type: 'POST',
                url: Config.base_url+'opd/api/opd_api/deleterelated',
                data: {
                    'id': id
                },
                success: function (data) {
...
function deleteItem(type,id){
    swal({   
        ...
        if (isConfirm) {     
            ...
            $.ajax({
                type: 'POST',
                url: Config.base_url+'opd/api/opd_api/deleteitem',
                data: {
                    'id': id,
                    'type': type,
                    'opd_id': opd_id,
                ...
                success: function (data) {
                    ...
                    $(addItemConfig.liststock+type+'-'+id).remove();
                    checkAlertCategory();
                }
            });
        }
...
function loadModaledititem(stock_category_id,id){
 $("#modal_container").empty();
    modelurl = itemUrl.editmodeldrug_url+'/'+opd_id+'/'+id;
...
function loadModaleditRelated(stock_category_id,id){
 $("#modal_container").empty();  
    ...
    modelurl = itemUrl.editmodelRelateddrug_url+'/'+opd_id+'/'+id;
    model_id = addItemConfig.modalliststockdrug;
...
function loadModaladditem(stock_category_id,id){
 $("#modal_container").empty();
    ...
    modelurl = itemUrl.modeldrug_url+'/'+opd_id+'/'+id;
    model_id = addItemConfig.modalliststockdrug;
...
function checkAlertCategory(){
    ...
    finalmodalliststockdrug : '#finalmodalliststockdrug',
    finalmodalliststockother : '#finalmodalliststockother',
    btnnext : '.timeline-nav-button--next',
    checkbox : '#md_checkbox_',
    inputunit : '.inputunit',
    ...
    liststock : '#list-stock-',
    stock_url:config.base_url+"opd/api/opd_api/getStock",
    finalmodeldrug_url:config.base_url+"opd/api/opd_modal/final_model_list_drug",
    editmodelRelatedother_url:config.base_url+"opd/api/opd_modal/edit_model_related_list_other",
    finalmodelother_url:config.base_url+"opd/api/opd_modal/final_model_list_other",
};

var listOpd = [
    {},
    {'form':"history_form",'link':"savehistory"},
    {'form':"physical_form",'link':"savephsical"},
    {'form':"vaccine_form",'link':"savevaccine"},
    {'form':"eye_form",'link':"saveeye"},
    {'form':"skin_form",'link':"saveskin"},
    {'form':"lab_form",'link':"savelab"},
    {'form':"diff_diagnosis_form",'link':"savediffdiagnosis"},
    {'form':"final_diagnosis_form",'link':"savefinaldiagnosis"},
    {'form':"prognosis_form",'link':"saveprognosis"},
    {'form':"treatment_form",'link':"savetreatment"},
    {'form':"notifications_form",'link':"savenotifications"},
    {'form':"listdrug_form",'link':"savelistdrug"}
];
...
function timelineNext(){
    $(OpdConfig.btnnext).click();
}


function submitOpd(step,link){

    var myform = document.getElementById(listOpd[step].form);
    var form_data = new FormData(myform);

    if(step == 2){
        form_data.append("queue_uid", queue_uid);
    }else{
        form_data.append("opd_id", opd_id);
    }

    $.ajax({    
        url: Config.base_url+'opd/api/opd_api/'+listOpd[step].link,
        dataType: 'text',
        cache: false,
        contentType: false,
        processData: false,
...
function editOpd(step,link){

    var myform = document.getElementById(editlistOpd[step].form);
    var form_data = new FormData(myform);
    ...
    $.ajax({    
        url: Config.base_url+'opd/api/opd_api/'+editlistOpd[step].link,
        dataType: 'text',
        cache: false,
        contentType: false,
        processData: false,
...
function submitLab(lab){
    ...
    $.ajax({    
        url: Config.base_url+'opd/api/opd_api/'+subLab[lab].link,
        dataType: 'text',
        cache: false,
        contentType: false,
        processData: false,
...
function finalloadModal(stock_category_id,id){
 $("#modal_container").empty();
 if(stock_category_id == 1){
    modelurl = OpdConfig.finalmodeldrug_url+'/'+opd_id+'/'+id;
    model_id = OpdConfig.finalmodalliststockdrug;
 }else{
    modelurl = OpdConfig.finalmodelother_url+'/'+opd_id+'/'+id;
    model_id =OpdConfig.finalmodalliststockother;
 }
...
function checkInput($uid){
    $(OpdConfig.checkbox+$uid).change(function () {
        if(!this.checked) {
            $(OpdConfig.inputunit+$uid).addClass("disabled").prop("disabled", true);
            $(OpdConfig.inputunit+$uid).val('');
        }else{
            $(OpdConfig.inputunit+$uid).removeClass("disabled").prop("disabled", false);
        }
    });
}

function checkInputRadio($uid){
    $(OpdConfig.checkbox+$uid).change(function () {
        if(!this.checked) {
            $(OpdConfig.inputradio+$uid).addClass("disabled").prop("disabled", true);
            $(OpdConfig.inputradio+$uid).prop("checked", false);
        }else{
            $(OpdConfig.inputradio+$uid).removeClass("disabled").prop("disabled", false);
        }
    });
}

function checkInputRadioInhouseLab($uid){
    $(OpdConfig.labinhouse_checkbox_category_+$uid).change(function () {
        if(!this.checked) {
            $(OpdConfig.inputradiolab+$uid).addClass("disabled").prop("disabled", true);
            $(OpdConfig.inputradiolab+$uid).prop("checked", false);
            $(OpdConfig.inputradiolab+$uid).val('');

            $(OpdConfig.labcategory_checkbox+$uid).prop("checked", false);
            $(OpdConfig.labcategory_checkbox+$uid).prop("disabled", true);
            $(OpdConfig.labcategory_input+$uid).val('');
            $(OpdConfig.labcategory_input+$uid).prop("disabled", true);

            $(OpdConfig.labinhouse_link+$uid).prop("checked", false);
        }else{

            $(OpdConfig.inputradiolab+$uid).removeClass("disabled").prop("disabled", false);
            $(OpdConfig.labcategory_checkbox+$uid).prop("disabled", false);

            $(OpdConfig.labinhouse_link+$uid).prop("checked", true);
        }
    });
}

function checkInputRadioOutLab($uid){
    $(OpdConfig.outlab_checkbox_category_+$uid).change(function () {
        if(!this.checked) {
            $(OpdConfig.inputradiolab+$uid).addClass("disabled").prop("disabled", true);
            $(OpdConfig.inputradiolab+$uid).prop("checked", false);
            $(OpdConfig.inputradiolab+$uid).val('');
            ...
            $(OpdConfig.outlab_link+$uid).prop("checked", false);
        }else{

            $(OpdConfig.inputradiolab+$uid).removeClass("disabled").prop("disabled", false);
            $(OpdConfig.labcategory_checkbox+$uid).prop("disabled", false);

            $(OpdConfig.outlab_link+$uid).prop("checked", true);
        }
    });
}

function checkInputRadiooutLabSub($uid){
    $(OpdConfig.outlab_checkbox_stock_+$uid).change(function () {
        if(!this.checked) {
            $(OpdConfig.inputradiolabsub+$uid).addClass("disabled").prop("disabled", true);
            $(OpdConfig.inputradiolabsub+$uid).prop("checked", false);
            $(OpdConfig.inputradiolabsub+$uid).val('');
        }else{
            $(OpdConfig.inputradiolabsub+$uid).removeClass("disabled").prop("disabled", false);
        }
    });
}

function checkInputRadioInhouseLabSub($uid){
    $(OpdConfig.labinhouse_checkbox_stock_+$uid).change(function () {
        if(!this.checked) {
            $(OpdConfig.inputradiolabsub+$uid).addClass("disabled").prop("disabled", true);
            $(OpdConfig.inputradiolabsub+$uid).prop("checked", false);
            $(OpdConfig.inputradiolabsub+$uid).val('');
...
$('#petnote').on('change',function(){
    var petnote = $(this).val();
    $.ajax({
        type: 'POST',
        url: Config.base_url+'opd/api/opd_api/savenote',
        data: {
            'petnote': petnote,
            'pet_uid': pet_uid
        },
...
$('#petfollow').on('change',function(){
    var petfollow = $(this).val();
    $.ajax({
        type: 'POST',
        url: Config.base_url+'opd/api/opd_api/savefollow',
        data: {
            'petfollow': petfollow,
            'pet_uid': pet_uid
        },
...
$('#pet_temperature').on('change',function(){
    var pet_temperature = $(this).val();
    $.ajax({
        type: 'POST',
        url: Config.base_url+'opd/api/opd_api/savetemperature',
        data: {
            'pet_temperature': pet_temperature,
            'queue_uid': queue_uid
        },
        success: function (data) {
...
$('#pet_weight').on('change',function(){
  	var pet_weight = $(this).val();
  	$.ajax({
        type: 'POST',
        url: Config.base_url+'opd/api/opd_api/saveweight',
        data: {
