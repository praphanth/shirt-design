...
var petConfig = {
	expense_url:config.base_url+"pet/api/pet_api/getexpense",
	appointment_url:config.base_url+"pet/api/pet_api/getappointment",
...
function startDatatable(){
	...
	if($(petConfig.datatableexpense).length){
		expense_table = $(petConfig.datatableexpense).DataTable({
			"sPaginationType": "full_numbers",
			"dom": ' tpi', 
            order: [[0, 'DESC']],
			"columns": [
			...
			"ajax": {
			"url": petConfig.expense_url+'/'+pet_uid,
			"data": function ( d ) {
	...
	if($(petConfig.datatableappointment).length){
			appointment_table = $(petConfig.datatableappointment).DataTable({
			"sPaginationType": "full_numbers",
			"dom": 'tpi', 
			"ordering": false,
			"columns": [
			...
			"ajax": {
			"url": petConfig.appointment_url+'/'+pet_uid,
			"data": function ( d ) {
	...
	if($(petConfig.datatableadmit).length){
	
        admit_table = $(petConfig.datatableadmit).DataTable({
            "sPaginationType": "full_numbers",
            "dom": ' tpi', 
            "columns": [
                    {"width": "15%" },
            ...
            "ajax": {
            "url": petConfig.admit_url+'/'+pet_uid,
            "data": function ( d ) {
